...
final class RknnSession implements AutoCloseable {
    private static final String TAG = "LipTalkRKNN";
    private static final boolean LIBRARY_LOADED;

    static {
        boolean loaded;
        try {
            System.loadLibrary("liptalk_rknn");
            loaded = true;
        } catch (Throwable error) {
            Log.i(TAG, "RKNN native runtime unavailable; using ONNX fallback", error);
            loaded = false;
        }
    ...
    static RknnSession tryCreate(Context context, String assetName, int... expectedOutputSizes) {
        if (!LIBRARY_LOADED || !DevicePlatform.isRockchip()) return null;
        try {
            ...
            Log.i(TAG, "RKNN model unavailable; using ONNX fallback: " + assetName, error);
            return null;
        }
    }
